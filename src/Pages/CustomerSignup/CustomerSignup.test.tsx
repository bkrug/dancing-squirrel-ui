...
test('User submitted a form and the backend reported success. Expect the form to become invisible and a success message to be displayed instead.', async () => {
  ...
  const responseFromHttpSubmit = Promise.resolve({ isSuccess: true } as FormResponse<TrainingRequestValidationFailures>);
  (submitFormikForm as jest.Mock).mockReturnValue(responseFromHttpSubmit);
  ...
  await act(async () => responseFromHttpSubmit);
...
test('The backend reports a validation failure. Expect the form to remain visible.', async () => {
  ...
  const responseFromHttpSubmit = Promise.resolve({
    isSuccess: false,
